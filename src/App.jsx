...
function App() {
  ...
  // Effect 3: The Single "Reactor" - This one effect controls all visual map state.
  useEffect(() => {
    if (!map.current?.isStyleLoaded()) return;

    // A: Update feature-state for all pubs
    for (const pub of allPubs) {
        map.current.setFeatureState({ source: 'pubs-source', id: pub.id }, {
            hovered: pub.id === hoveredPubId,
            selected: pub.id === selectedPubId
        });
    }

    // B: Update paint properties for focus/opacity effect
    ...
    // D: Fly-to selected pub
    if (selectedPub && selectedPub.id !== lastSelectedId.current) {
  ...
  }, [allPubs, hoveredPubId, selectedPubId, selectedPub, crawlPubIds]);


  // Effect 4: Manages crawl route geojson data
  useEffect(() => {
    if (!map.current?.isStyleLoaded()) return;
  ...
  const handleLogVisit = async (pubId, options = {}) => {
    ...
    else { await refetchPubs(options.navigateOnSuccess ? pubId : null); setNotification({ message: `Visit logged for ${pubName}!`, type: 'success' }); }
    setIsProcessing(false);
  };
  ...
  const handleRemoveVisit = async (pubId, visitId, options = {}) => {
    ...
    else { await refetchPubs(options.navigateOnSuccess ? pubId : null); setNotification({ message: `Last visit removed for ${pubName}.`, type: 'success' }); }
    setIsProcessing(false);
  };
