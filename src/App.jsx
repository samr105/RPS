...
function App() {
  ...
  useEffect(() => {
    ...
    map.current.on('load', async () => {
      ...
      // *** FIX #1: THE LABEL LAYER ***
      // 'text-field' is static. 'text-opacity' is a paint property and can use feature-state.
      map.current.addLayer({
        id: 'pub-labels', type: 'symbol', source: 'pubs-source',
        layout: {
          'text-field': ['get', 'name'], // Always get the name.
          'text-font': ['Open Sans Bold', 'Arial Unicode MS Bold'],
          'text-offset': [0, 1.25], 'text-anchor': 'top',
          'text-allow-overlap': true, 'text-ignore-placement': true, // Important for hover
        },
        paint: {
          'text-color': '#222', 'text-halo-color': '#fff', 'text-halo-width': 1.5,
          'text-opacity': ['case', ['boolean', ['feature-state', 'hover'], false], 1.0, 0.0], // Show/hide based on hover.
          'text-opacity-transition': { duration: 200 },
        }
      });
      
      // *** FIX #2: THE EVENT LISTENERS ***
      // They are self-contained and manage the direct visual feedback.
      ...
      map.current.on('mousemove', 'pubs-layer', (e) => {
        if (e.features.length > 0) {
          ...
          if (newHoverId !== currentHoverId) {
            ...
            setHoveredPubId(newHoverId); // Set React state to trigger dimming effect
      ...
      map.current.on('mouseleave', 'pubs-layer', () => {
        ...
        if (currentHoverId !== null) {
          ...
          setHoveredPubId(null); // Clear React state to undo dimming effect
          currentHoverId = null;
        }
      });
      ...
      setAllPubs(pubData); // Set React state for the UI
      allPubsRef.current = pubData; // Set the ref for map click events
