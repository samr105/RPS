// src/App.jsx - FINAL, CORRECTED VERSION FOR UI STYLING
...
function App() {
  ...
  // This function fetches the initial data and is correct
    ...
    const pubsForMap = data
      .map(pub => {
        ...
        try {
          ...
          return {
            type: 'Feature',
            id: pub.id, // THE CRUCIAL ID PROPERTY
  ...
  useEffect(() => {
    ...
    map.current.on('load', async () => {
      ...
      map.current.addSource('pubs-source', {
        ...
        promoteId: 'id' // THIS IS KEY: It makes 'id' available for hover effects
      });

      const interactiveLayers = ['unvisited-pubs-layer', 'visited-pubs-layer'];
      
      // === REVISED MARKER LAYERS ===
      ...
      map.current.on('mousemove', interactiveLayers, (e) => {
        if (e.features.length > 0) {
          ...
          map.current.setLayoutProperty('pub-labels', 'visibility', 'visible');
          map.current.setFilter('pub-labels', ['==', 'id', hoveredPubId]);
        }
      });
      ...
      map.current.on('mouseleave', interactiveLayers, () => {
        ...
        hoveredPubId = null;
        map.current.setLayoutProperty('pub-labels', 'visibility', 'none');
      });
      ...
      map.current.on('click', interactiveLayers, (e) => {
        ...
        new maplibregl.Popup({ offset: [0, -15] })
          ...
          .setHTML(createPopupHTML(feature.properties, feature.geometry.coordinates))
          .addTo(map.current);
