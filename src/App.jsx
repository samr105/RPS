...
const pintSVG = `data:image/svg+xml;charset=utf-8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18.5 3h-13L4 21h16L18.5 3z"></path></svg>')}`;
...
function App() {
  ...
  useEffect(() => {
    ...
    map.current.on('load', async () => {
      ...
      map.current.addSource('pubs-source', { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
      ...
      map.current.on('click', 'pubs-icons-layer', (e) => { if (e.features.length > 0) { const clickedPub = allPubs.find(p => p.id === e.features[0].id); eventHandlersRef.current.handlePubClick(clickedPub); } });
      ...
      map.current.getSource('pubs-source').setData({ type: 'FeatureCollection', features }); setIsLoading(false);
    });
  }, [handleDataUpdate]);
