...
function App() {
  ...
  const previousSelectedPubId = useRef(null); // Keep track of the last selected pub
    ...
    map.current.on('load', async () => {
      ...
      map.current.addLayer({
        ...
        paint: {
          ...
          // Radius: Largest for hover, large for selected, default otherwise
          'circle-radius': ['case',
            ['boolean', ['feature-state', 'hover'], false], 11,
            ['boolean', ['feature-state', 'selected'], false], 9,
            7
          ],
          // Stroke: Prominent blue for selected, white otherwise
      ...
      map.current.addLayer({
        ...
        paint: {
          ...
          // Show label on hover OR if selected
          'text-opacity': ['case',
            ['any',
  ...
  useEffect(() => {
    ...
    // Clear the selection state from the previously selected pub
    if (previousSelectedPubId.current !== null) {
        map.current.setFeatureState({ source: 'pubs-source', id: previousSelectedPubId.current }, { selected: false });
    }

    // Apply the selection state to the newly selected pub
    if (selectedPub !== null) {
  ...
  // Effect to control map-wide dimming for hover and crawls
  useEffect(() => {
