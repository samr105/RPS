...
function App() {
  // All state and ref declarations are correct
  ...
  // Memoized fetch function for pub data
  const fetchPubs = useCallback(async () => {
    ...
    // This part is crucial: it only UPDATES data on an existing source
    if (map.current && map.current.getSource('pubs-source')) {
      map.current.getSource('pubs-source').setData({ type: 'FeatureCollection', features: pubsForMap });
    }
    setIsLoading(false);
  }, []); // Empty dependency array means this function definition never changes

  // Fetch reviews function remains unchanged
  const fetchReviewsForPub = useCallback(async (pubId) => { const { data, error } = await supabase.from('visits').select('*').eq('pub_id', pubId).order('visit_date', { ascending: false }); if (error) { setNotification({ message: 'Could not fetch reviews for this pub.', type: 'error' }); setSelectedPubReviews([]); } else { setSelectedPubReviews(data); } }, []);
  useEffect(() => { if (selectedPub) { fetchReviewsForPub(selectedPub.id); } else { setSelectedPubReviews([]); } }, [selectedPub, fetchReviewsForPub]);

  // *** THE CORE FIX: A DEDICATED INITIALIZATION EFFECT ***
  // This useEffect runs exactly ONCE, when the component mounts.
  useEffect(() => {
    ...
    // The 'load' event fires once after the map's resources are ready.
    // All one-time setup should happen inside this event.
    map.current.on('load', () => {
      // 1. Add the source for our pub data (initially empty)
      map.current.addSource('pubs-source', { type: 'geojson', data: { type: 'FeatureCollection', features: [] }, promoteId: 'id' });

      // 2. Add the single, powerful layer for all pubs
      ...
      // 3. Add the layer for labels, also controlled by feature state
      ...
      // 4. Set up all permanent event listeners
      let currentHoverId = null; // A local variable within the closure is more efficient here
  ...
  // This "Style Controller" effect now has a stable map to work with.
  // It correctly depends on the state that drives visual changes.
  useEffect(() => {
